...
import AbyssiniaSecondTrenchColumn from "~/components/abyssinia_second_trench_column.vue";
import AbyssiniaThirdTrenchColumn from "~/components/abyssinia_third_trench_column.vue";
import HibretThirdTrenchColumn from "~/components/hibret_third_trench_column.vue";
import AbyssiniaforthTrenchColumn from "~/components/abyssinia_forth_trench_column.vue";
import HibretforthTrenchColumn from "~/components/hibret_forth_trench_column.vue";
import AwashforthTrenchColumn from "~/components/awash_forth_trench_column.vue";
import DashenforthTrenchColumn from "~/components/dashen_forth_trench_column.vue";
...
onMounted(async () => {
  ...
  try {
    ...
    console.error("Failed to fetch Abyssinia 2nd Trench data:", err);
    showAbyssiniaSecondTrench.value = false;
  }
  ...
  try {
    ...
    showHibretThirdTrench.value = hibretThirdAmount > 0;
  } catch (err) {
    console.error("Failed to fetch 3rd Trench data:", err);
    showAbyssiniaThirdTrench.value = false;
  ...
  try {
    ...
    showAwashforthTrench.value = getVal("Awash Trench Amount") > 0;
    showDashenforthTrench.value = getVal("Dashen Trench Amount") > 0;
  } catch (err) {
    console.error("Failed to fetch 4th Trench data:", err);
    showAbyssiniaforthTrench.value = false;
    showHibretforthTrench.value = false;
    showAwashforthTrench.value = false;
    showDashenforthTrench.value = false;
  }

  // Fetch dynamic headers for BDS Status
  try {
  ...
  // Fetch dynamic headers for Credit Disbursement
  try {
    ...
    >
      ...
      <div class="max-w-7xl mx-auto mt-16 mb-16">
        ...
        <div class="mb-8">
          ...
          <template v-if="showAbyssiniaSecondTrench">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div
                class="bg-white rounded-2xl shadow p-6 flex items-center justify-center min-h-[200px]"
              >
                <client-only>
                  <AbyssiniaSecondTrenchColumn />
                </client-only>
              </div>
              <div
                class="bg-white rounded-2xl shadow p-6 flex items-center justify-center min-h-[200px]"
        ...
        <!-- 4th Trench -->
        <div v-if="showforthTrenchSection" class="mb-8">
          <h2 class="text-2xl font-bold text-gray-800 mb-4 text-center">
